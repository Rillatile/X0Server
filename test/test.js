  ...
  it('Проверка создания лобби', (done) => {
    client1.on('connect', () => {
      ...
      client1.emit('getUniqueID', (data1) => {
        ...
        client1.on('lobbyCreated', (data2) => {
          expect(data2).to.have.property('id');
          expect(data2.id).to.be.a('string', 'Идентификатор не является строкой');
          expect(data2.id.length).to.not.equal(0, 'Идентификатор пустой');
          expect(data2.id).to.equal('1');
          done();
        });
      });
    });
  ...
  it('Проверка корректного подключения к лобби', (done) => {
    client1.on('connect', () => {
      ...
      client1.emit('getUniqueID', (data1) => {
        ...
        client1.on('lobbyCreated', (data2) => {
          const secondClient = io(`http://localhost:${port.toString()}`);

          secondClient.on('connect', () => {
            let id2 = '';

            secondClient.emit('getUniqueID', (data3) => {
              id2 = data3.id;
              secondClient.emit('joinLobby', {
                lobbyID: data2.id, password: '', clientID: id2, clientName: 'Ivan',
              });

              secondClient.on('successJoin', (data4) => {
                expect(data4.id).to.equal(data2.id);
                done();
              });
            });
          });
    ...
    client1.on('connect', () => {
      ...
      client1.emit('getUniqueID', (data1) => {
        ...
        client1.emit('joinLobby', {
          lobbyID: '3', password: '', clientID: id, clientName: 'Roman',
        });

        client1.on('successJoin', (data2) => {
          expect(1).to.equal(2, 'От сервера пришло сообщение об успешном подключении');
          expect(data2.id).to.equal('3', 'От сервера пришло сообщение об успешном подключении');
          done();
        });
  ...
  it('Проверка попытки подключения к лобби, используя неправильный пароль', (done) => {
    client1.on('connect', () => {
      ...
      client1.emit('getUniqueID', (data1) => {
        ...
        client1.on('lobbyCreated', (data2) => {
          client1.emit('joinLobby', {
            lobbyID: data2.id, password: '123', clientID: id, clientName: 'Roman',
          });

          client1.on('successJoin', (data3) => {
            expect(1).to.equal(2, 'От сервера пришло сообщение об успешном подключении');
            expect(data3.id).to.equal(data2.id, 'От сервера пришло сообщение об успешном подключении');
            done();
          });
    ...
    client1.on('connect', () => {
      ...
      client1.emit('getUniqueID', (data1) => {
        ...
        client1.on('lobbyCreated', (data2) => {
          client1.emit('joinLobby',
            {
              lobbyID: data2.id, password: '', clientID: id, clientName: 'Roman',
            });

          client1.on('successJoin', (data3) => {
            client1.emit('leaveLobby', { lobbyID: data3.id, clientID: id });

            client1.on('lobbiesUpdated', (data4) => {
              const idx = data4.findIndex((element) => element.id === data3.id);
...
describe('Проверка некорректной партии', () => {
  ...
  before((done) => {
    ...
    client1.emit('getUniqueID', (data1) => {
      ...
      client1.on('lobbyCreated', (lobbyInfo) => {
        client1.emit('joinLobby', {
          lobbyID: lobbyInfo.id, password: '1234', clientID: id1, clientName: 'Roman',
        });
        client1.on('successJoin', () => {
          client2.emit('getUniqueID', (data3) => {
            id2 = data3.id;
            client2.emit('joinLobby', {
              lobbyID: lobbyInfo.id, password: '1234', clientID: id2, clientName: 'Ivan',
            });
  ...
  const makeIncorrectMove = (player, point) => {
    ...
    player.client.on('moveIsCorrect', () => {
